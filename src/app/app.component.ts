...
export class AppComponent implements OnInit {
  ...
  segmentAngle: number = 38;  // Cada segmento cobre 45 graus (180/4)
  initialOffset: number = 222; // Começar no lado esquerdo (180 graus)
  ...
  getColor(index: number): string {
    ...
    // Calcula a fração do segmento (0 a 1)
    const fraction = (index + 1) / totalSegments;
    
    // Calcula o Hue de 0 a 60 para transição de vermelho a amarelo
    const hue = 60 * fraction;
  ...
  polarToCartesian(centerX: number, centerY: number, radius: number, angle: number): { x: number, y: number } {
    console.log(angle, "ASD")
    const angleRad = (angle) * Math.PI / 180.0; // Usar o ângulo diretamente, sem ajustes
// radius 125
console.log(radius * Math.cos(angleRad), radius * Math.sin(angleRad))
    return {
